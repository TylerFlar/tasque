...
from tasque.chains.scheduler import resume_interrupted_chains
...
async def _run_async(*, drainer_poll_seconds: float) -> None:
    ...
    # Re-invoke any chain mid-flight at last shutdown. Run in a thread so
    # synchronous LLM round-trips don't block the asyncio loop while the
    # bot keeps responding to gateway events.
    async def _resume() -> None:
        try:
            resumed = await asyncio.to_thread(resume_interrupted_chains)
            if resumed:
                log.info("serve.resumed_chains", count=len(resumed))
        except Exception:
            log.exception("serve.resume_chains_failed")

    resume_task = asyncio.create_task(_resume(), name="tasque-resume-chains")
